...
// Moving cats new logic

(function movableCats() {

  const CAT_VALUES = [55, 56, 57, 58, 59];
  const WALKABLE = 10;
  const INTERVAL = 700;
  ...
  function getCatPositions() {
    ...
    for (let i = 0; i < gameBlocks.length; i++) {
      if (CAT_VALUES.includes(gameBlocks[i])) {
        cats.push({ index: i, value: gameBlocks[i] });
      }
    }
    return cats;
  }
  ...
  function getNeighbors(index) {
    ...
    return [
      ...
      .map(p => p.x + p.y * gridSize);
  }

  function moveCatsOnce() {
    if (gameOver) return;

    const cats = getCatPositions();

    cats.forEach(cat => {
      const neighbors = getNeighbors(cat.index)
        .filter(i => gameBlocks[i] === WALKABLE);

      if (!neighbors.length) return;

      const target = neighbors[Math.floor(Math.random() * neighbors.length)];

      gameBlocks[cat.index] = WALKABLE;
      gameBlocks[target] = cat.value;
  ...
  setInterval(moveCatsOnce, INTERVAL);
