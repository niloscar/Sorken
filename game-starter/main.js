...
window.addEventListener('DOMContentLoaded',async function () {
  ...
  let rockford = document.getElementById('baddie1'),
    ...
    gridSize = state.mapData.meta.width,// Grid size 24x24
    gameStarted = false,
    livesCount = Number(),
    lastItemIndex,
  ...
  if (state.mapData.meta.lightsOut) area.classList.add('lightsout');
  ...
  function openDoor(target,collected) {
    if (!state.mapData.meta.lastDoor) return;
    if (collected >= target) {
      console.log('Door open!');
      gameBlocks[state.mapData.meta.lastDoor] = 10;
    }
  }
  ...
  // Timelimit
  ((timeLimit = state.mapData.meta.timeLimit + ':00') => {
